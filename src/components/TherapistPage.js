...
// import styles from './therapist.css';
...
export default class TherapistPage extends Component {
    ...
    async componentDidMount() {
        ...
        const urlPatientsForTherapist = 'http://127.0.0.1:8000/api/patients?therapist_id='+user_id;
                    ...
                    {this.state.raw_data.map(item =>
                         <div key={item["id"]}>
                            ...
                            <label> Username: {item["username"]}</label>
                             ...
                             <br/>
                             <a href={`/patient/${item["id"]}`}> Go to dashboard for  { item["username"]}
                             </a>
                             <br/>
                             <a href={`/chats/${item["id"]}`}> Chat with  { item["username"]}
                             </a>
                             {/* </button> */}
