...
export default class Messages extends Component {
    ...
    handleSend = ()=>{
        if(this.state.msg != '')
        {
            ...
            $.post("http://127.0.0.1:8000/api/chat_messages", { author_id: localStorage.user_id, text: this.state.msg , recipient_id: this.state.user2})
            .then(
                res=>{
    ...
    async componentDidMount() {
        ...
        for (const val of dataMessagesForUser){
            console.log("VAL::::",val);
            if(val["author_id"] == user_id && val["recipient_id"] == this.state.user2){
